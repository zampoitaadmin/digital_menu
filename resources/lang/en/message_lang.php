<?php
return [
    'common_error_500' => "Something went wrong.",
    'common_error_505' => "Something went wrong.",
    'common_error_400' => "Something went wrong.",

    'connecting_text_1' => "SSO Connecting...",
    'connecting_text_2' => "Connecting Custom Menu Server...",

    'dashboard_text_1' => 'Dashboard',
    'preview_text_1' => 'Preview',

    'module_category' => "Category",
    'form_input_category' => "category name (EN)",
    'form_input_categorySP' => "category name (SP)",

    'api_token_' => "Something went wrong.",
    'form_input_placeholder' => "Enter :formInput", //Enter Category Name (SP)


    //PageWIse
    /*#TODo Category */
    'category_text_1' => "Categories",
    'category_text_2' => "Choose Category",
    'category_text_3' => "Create Own Category",
    'category_text_4' => "Select Your Category",
    'category_text_5' => "Your Selected Category",
    'category_text_6' => "Create Your Own Category",
    'category_text_7' => "Update Category",

    'branding_text_1' => "Branding",
    'branding_color_main' => "Main Color",
    'branding_color_secondary' => "Secondary Color",
    'branding_color_third' => "Third Color",
    'branding_color_font' => "Font Color",
    'branding_logo' => "Brand Logo",
    'branding_revert_to_Default' => "Revert To Default",

    'setting_text_1' => "Setting",
    'setting_website_template' => "Active Menu In Website Template",
    'setting_url_externally' => "use the below URL to link your menu externally",
    'setting_delivery_info_text' => "Enter Your Delivery Information Below",
    'setting_standard_delivery_charge' => "Standard Delivery Charge",
    'setting_minimum_delivery_charge' => "Minimum Order For Delivery",
    'setting_free_delivery_charge' => "Free Delivery Charge on Order Over",

    /* Product */
    'product_text_1' => "Products",
    'product_text_2' => "Allergy items selected",
    'product_menu_item_no' => "MENU ITEM NO",
    'product_add_product' => "Add Product",
    'product_update_product' => "Update Product",

    'lbl_category' => "Category",
    'lbl_product_name' => "Product Name",
    'lbl_product_description' => "Product Description",
    'lbl_upload_file' => "Upload File",
    'lbl_product_price' => "Product Price",
    'lbl_tapa' => "Tapa",
    'lbl_12r' => "1/2 R",
    'lbl_1r' => "1 R",
    'lbl_fixed' => "Fixed",
    'lbl_allergy' => "Allergy",
    'lbl_status' => "Status",

    /* Fixed Menu */
    'lbl_fixed_menu' => "Fixed Menu",
    'lbl_change_category_name' => "Change Category Name",
    'lbl_menu_description_conditions' => "Menu Description And Conditions",
    'lbl_starters' => "Starters",
    'lbl_main_course' => "Main Course",
    'lbl_deserts' => "Deserts",
    'lbl_select_allergy' => "Select Allergy",
    'lbl_fixed_menu_price' => "Fixed Menu Price",

    /* Footer Lang Labels From Old */
    'legal' => "Legal",
    'termsandconditions' => "Terms & Conditions",
    'com_ter_co' => "TERMS & CONDITIONS - COMPETITIONS",
    'privacy_policy' => "Privacy Policy",
    'cookies_info_text_f' => "Cookie Info",
    'company' => "Company",
    'About_Us' => "About us",
    'why_zampoita' => "Why Zampoita",
    'pricing' => "Pricing",
    'blog' => "Blog",
    'services' => "Services",
    'prem_listing' => "Premium Listing",
    'zam_design' => "Zampoita Design",
    'web_design' => "Website Design",
    'tbl_reserver' => "Table Reserving & Food Ordering",
    'competitions_' => "Competitions",
    'contact' => "Support",
    'signup' => "Signup",
    'client_login' => "Client login",
    'copyright' => "Copyright",
    'lang_eng' => "English",
    'lang_spanish' => "Spanish",
];
?>