<nav class="navbar ms-navbar">
    ...
    <ul class="ms-nav-list ms-inline mb-0" id="ms-nav-options">
        ...
        <li class="ms-nav-item dropdown1">
            <a href="{{ route('home') }}" target="_blank" class="text-disabled" id="" data-toggle="" aria-haspopup="" aria-expanded=""><i class="fas fa-external-link-square-alt"></i> Admin Order</a>
        </li>
        <li class="ms-nav-item ms-nav-user dropdown">
            <a href="#" id="userDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            @php
                $profile_image = url('uploads/admin/').'/'.auth()->guard("admin")->user()->profile_image ?? "";
            @endphp
            <img class="ms-user-img ms-img-round float-right" src="{{ $profile_image }}" alt="people" style="height: 30px;" >
            </a>
            <ul class="dropdown-menu dropdown-menu-right user-dropdown" aria-labelledby="userDropdown">
                <li class="dropdown-menu-header">
                    <h6 class="dropdown-header ms-inline m-0"><span class="text-disabled">Welcome, {{ auth()->guard("admin")->user()->first_name }} {{ auth()->guard("admin")->user()->last_name }}</span></h6>
                </li>
                <li class="dropdown-divider"></li>
                <li class="ms-dropdown-list">
                    <a class="media fs-14 p-2" href="{{ route('profile-admin') }}"> <span><i class="flaticon-user mr-2"></i> Profile</span></a>
                ...
                <li class="dropdown-menu-footer">
                    <a class="media fs-14 p-2" href="{{ route('adminlogout') }}"> <span><i class="flaticon-shut-down mr-2"></i> {{ __('message_lang.LOGOUT') }}</span>
                    </a>
                </li>
            </ul>
        </li>
